...
match_path_test_() ->
    [
     {"exact match",
      ?_assertEqual({true, #{}}, httpd_router:match_path("/hello", "/hello"))},
     {"root match",
      ?_assertEqual({true, #{}}, httpd_router:match_path("/", "/"))},
     {"single param",
      ?_assertEqual({true, #{id => "42"}}, httpd_router:match_path("/users/:id", "/users/42"))},
     {"multiple params",
      ?_assertEqual({true, #{org => "acme", id => "7"}},
                    httpd_router:match_path("/orgs/:org/users/:id", "/orgs/acme/users/7"))},
     {"no match - different length",
      ?_assertEqual(false, httpd_router:match_path("/users/:id", "/users/42/posts"))},
     {"no match - different segment",
      ?_assertEqual(false, httpd_router:match_path("/users/:id", "/posts/42"))},
     {"empty path no match",
...
route_registration_test_() ->
    {setup,
     ...
     fun(_) ->
         [
          {"add and find simple route",
           fun() ->
               TableName = test_routes_simple,
               {ok, _} = httpd_router_server:create_table(TableName),
               Handler = fun(_Ctx) -> {status, 200} end,
               ok = httpd_router_server:add_route(TableName, "GET", "/hello", Handler, []),
               {ok, Route} = httpd_router:find_route("GET", "/hello", TableName),
               ?assertEqual("GET", Route#route.method),
               ?assertEqual("/hello", Route#route.path_pattern),
               ?assertEqual(Handler, Route#route.handler)
           end},
          {"find route with params",
           fun() ->
               TableName = test_routes_params,
               {ok, _} = httpd_router_server:create_table(TableName),
               Handler = fun(_Ctx) -> {status, 200} end,
               ok = httpd_router_server:add_route(TableName, "GET", "/users/:id", Handler, []),
               {ok, Route} = httpd_router:find_route("GET", "/users/99", TableName),
          ...
          {"not found returns not_found",
           fun() ->
               ...
               {ok, _} = httpd_router_server:create_table(TableName),
               ?assertEqual(not_found, httpd_router:find_route("GET", "/nope", TableName))
           end},
          {"method mismatch returns not_found",
           fun() ->
               TableName = test_routes_method,
               {ok, _} = httpd_router_server:create_table(TableName),
               Handler = fun(_Ctx) -> {status, 200} end,
               ok = httpd_router_server:add_route(TableName, "POST", "/items", Handler, []),
               ?assertEqual(not_found, httpd_router:find_route("GET", "/items", TableName))
           end}
         ]
     end}.
...
crud_expansion_test_() ->
    {setup,
     ...
     fun(_) ->
         [
          {"CRUD expands to all methods",
           fun() ->
               TableName = test_routes_crud,
               {ok, _} = httpd_router_server:create_table(TableName),
               Handler = fun(_Ctx) -> {status, 200} end,
               ok = httpd_router_server:add_route(TableName, "CRUD", "/items", Handler, []),
               ...
               {ok, R2} = httpd_router:find_route("GET", "/items/1", TableName),
               ...
               {ok, R4} = httpd_router:find_route("PUT", "/items/1", TableName),
               ?assertEqual(replace, R4#route.action),
               {ok, R5} = httpd_router:find_route("PATCH", "/items/1", TableName),
               ?assertEqual(modify, R5#route.action),

               %% D -> DELETE /items/:id (delete)
               {ok, R6} = httpd_router:find_route("DELETE", "/items/1", TableName),
               ?assertEqual(delete, R6#route.action),

               %% OPTIONS
               {ok, R7} = httpd_router:find_route("OPTIONS", "/items", TableName),
               ?assertEqual(options, R7#route.action)
           end},
          {"partial CRUD: CR only",
           fun() ->
               TableName = test_routes_cr,
               {ok, _} = httpd_router_server:create_table(TableName),
               Handler = fun(_Ctx) -> {status, 200} end,
               ok = httpd_router_server:add_route(TableName, "CR", "/things", Handler, []),

               {ok, _} = httpd_router:find_route("POST", "/things", TableName),
               {ok, _} = httpd_router:find_route("GET", "/things", TableName),
               {ok, _} = httpd_router:find_route("GET", "/things/1", TableName),
               ?assertEqual(not_found, httpd_router:find_route("DELETE", "/things/1", TableName)),
               ?assertEqual(not_found, httpd_router:find_route("PUT", "/things/1", TableName))
           end}
         ]
     end}.
    ...
    [
     {"middlewares execute in order",
      fun() ->
          ...
          {ok, Result} = httpd_router:execute_middlewares([M1, M2], #{opaque => #{}}),
          ?assertEqual(done, maps:get(step1, Result)),
          ?assertEqual(done, maps:get(step2, Result))
      end},
     {"middleware short-circuits on error",
      fun() ->
...
http_roundtrip_test_() ->
    {setup,
     fun() ->
         ...
         httpd_router:table_add_route(httpd_roundtrip_routes,
                                      "GET", "/test/hello",
     ...
     fun({_Pid, Port}) ->
         ...
         [
          {"GET /test/hello returns 200 with text body",
           fun() ->
               {ok, {{_, 200, _}, _Headers, Body}} =
                   httpc:request(get, {BaseUrl ++ "/test/hello", []},
                                 [{timeout, 2000}], []),
               ?assertEqual("Hello!", Body)
           end},
          {"GET /test/user/:id returns JSON with param",
           fun() ->
               {ok, {{_, 200, _}, Headers, Body}} =
                   httpc:request(get, {BaseUrl ++ "/test/user/42", []},
                                 [{timeout, 2000}], []),
               ?assertMatch("application/json" ++ _,
          ...
          {"GET unknown path returns 404",
           fun() ->
               {ok, {{_, 404, _}, _Headers, _Body}} =
                   httpc:request(get, {BaseUrl ++ "/nonexistent", []},
                                 [{timeout, 2000}], [])
           end}
         ]
     end}.
...
options_test_() ->
    {setup,
     ...
     fun(_) ->
         [
          {"default option is no_match 404",
           fun() ->
          ...
          {"can set no_match to passthrough",
           fun() ->
               TableName = test_routes_opts2,
               {ok, _} = httpd_router_server:create_table(TableName, #{no_match => passthrough}),
               {ok, Opts} = httpd_router_server:get_options(TableName),
